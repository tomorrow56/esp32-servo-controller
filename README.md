...
1. `ESP32Servo`・`ArduinoJson`・`SimpleWiFiManager`・`Adafruit SSD1306`・`ESP32FwUploader` をArduino IDEにインストール
2. `web_api/src/esp32_servo_webapi_server/esp32_servo_webapi_server.ino` を書き込む
3. 初回起動時に `ESP32-ServoAP` APが立ち上がるので接続し、`http://192.168.4.1` でSSID・パスワードを設定
4. シリアルモニタまたはOLEDでIPアドレスを確認
5. `web_api/web_ui/script_controller_ui.html` または `web_api/web_ui/block_editor/index.html` をブラウザで開く
...
1. `ESP32Servo`・micro-ROS for Arduino・`SimpleWiFiManager`・`Adafruit SSD1306`・`ESP32FwUploader` をArduino IDEにインストール
...
4. `ros2/src/esp32_servo_microros/esp32_servo_microros.ino` を書き込む
5. 初回起動時に `ESP32-ServoAP` APが立ち上がるので接続し、`http://192.168.4.1` でSSID・パスワード・Agent IPを設定
6. `ros2 launch servo_controller servo_controller.launch.py` でノードを起動
7. `ros2/web_ui/script_controller_ros2.html` をブラウザで開く

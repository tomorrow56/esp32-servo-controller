...
async function testConnection() {
  ...
  try {
    const res = await fetch(`${url}/api/servo/status`,
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data.servos)) {
        data.servos.forEach(s => {
          if (s.channel >= 0 && s.channel < NUM_SERVOS) {

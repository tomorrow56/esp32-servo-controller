...
ESP32のIPアドレスを `192.168.1.136` とした例です。

### curl

```bash
# 単体サーボ制御 (CH0 を 90度)
curl -X POST http://192.168.1.136/api/servo/0 \
  -H "Content-Type: application/json" \
  -d '{"angle": 90}'

# 複数サーボ同時制御
curl -X POST http://192.168.1.136/api/servos \
  -H "Content-Type: application/json" \
  -d '{"servos": [{"channel": 0, "angle": 90}, {"channel": 1, "angle": 45}]}'

# 全サーボ状態取得
curl http://192.168.1.136/api/servos

# スクリプトアップロード & 実行
curl -X POST http://192.168.1.136/api/script/upload \
  -H "Content-Type: application/json" \
  -d '{"script": "servo 0 90\nwait 500\nservo 0 0"}'

curl -X POST http://192.168.1.136/api/script/execute

# 実行状態確認
curl http://192.168.1.136/api/script/status
```

### JavaScript (fetch)

```javascript
const BASE = 'http://192.168.1.136';

// 単体サーボ制御
await fetch(`${BASE}/api/servo/0`, {
  method: 'POST',
...
// 全サーボ状態取得
const res = await fetch(`${BASE}/api/servos`);
...
BASE = 'http://192.168.1.136'

# 単体サーボ制御
requests.post(f'{BASE}/api/servo/0', json={'angle': 90})

# 複数サーボ同時制御
requests.post(f'{BASE}/api/servos', json={
    'servos': [{'channel': 0, 'angle': 90}, {'channel': 1, 'angle': 45}]
})

# 全サーボ状態取得
r = requests.get(f'{BASE}/api/servos')
print(r.json())

# スクリプトアップロード & 実行
requests.post(f'{BASE}/api/script/upload', json={
    'script': 'servo 0 90\nwait 500\nservo 0 0'
})
requests.post(f'{BASE}/api/script/execute')
```
...
ESP32の`xTaskCreatePinnedToCore`関数を使用して、スクリプト実行専用のタスクをコア1で生成しています。`vTaskDelay()`を使用することでWiFiスタックとの競合を避けながら安全に`wait`コマンドを処理します。スタックサイズは32KBに設定されており、行データはヒープ上の`std::vector<String>`に確保されます。
...
ESP32の`WebServer`ライブラリ（`WebServer.h`）を使用して、各APIエンドポイントをルートハンドラとして登録しています。`loop()`内で`apiServer.handleClient()`を呼ぶことでノンブロッキングにリクエストを処理します。OTAサーバー（ポート8080）も同じ`loop()`内で`otaServer.handleClient()`として並行動作します。

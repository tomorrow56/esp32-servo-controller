...
1.  `esp32_webapi_script_server.ino` ファイルをArduino IDEで開きます。
...
`script_controller_ui.html` をブラウザで開いて使用します。
...
```
**注意**: `repeat`ブロックは現在、ESP32のコードではサポートされていません。ブラウザ側のパーサーで展開されてからアップロードする必要があります。（将来の拡張機能）
...
```
**注意**: `loop`ブロックも同様に、ESP32のコードではサポートされていません。
...
call <name> [args...]
```
**注意**: 関数定義も同様に、ESP32のコードではサポートされていません。

**現在の実装**: 現在のESP32ファームウェアは、`servo`, `servos`, `wait` の基本コマンドのみを1行ずつ実行するシンプルなインタプリタです。ループや関数は、Web UI側でスクリプトを前処理して、単純なコマンドの羅列に展開してからアップロードする必要があります。
...
`sample_scripts/` フォルダに、Web UIから読み込んで使用できるサンプルが含まれています。
...
### Wi-Fiに接続できない
- `esp32_webapi_script_server.ino` のSSIDとパスワードが正しいか確認してください。
